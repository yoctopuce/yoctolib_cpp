...
#include "yocto_counter.h"
#include "yapi/yjson.h"
#include "yapi/yapi.h"
#define  __FILE_ID__  "counter"
 ...
 */
YCounter* YCounter::FindCounter(string func)
{
    YCounter* obj = NULL;
    int taken = 0;
 ...
 * @noreturn
 */
int YCounter::registerValueCallback(YCounterValueCallback callback)
{
    string val;
    if (callback != NULL) {
...
int YCounter::_invokeValueCallback(string value)
{
    if (_valueCallbackCounter != NULL) {
        _valueCallbackCounter(this, value);
    } else {
 ...
 * @noreturn
 */
int YCounter::registerTimedReportCallback(YCounterTimedReportCallback callback)
{
    YSensor* sensor = NULL;
    sensor = this;
    if (callback != NULL) {
...
int YCounter::_invokeTimedReportCallback(YMeasure value)
{
    if (_timedReportCallbackCounter != NULL) {
        _timedReportCallbackCounter(this, value);
    } else {
...
YCounter *YCounter::FirstCounter(void)
{
    vector<YFUN_DESCR>   v_fundescr;
    YDEV_DESCR             ydevice;
    string              serial, funcId, funcName, funcVal, errmsg;

    if(YISERR(YapiWrapper::getFunctionsByClass("Counter", 0, v_fundescr, sizeof(YFUN_DESCR), errmsg)) ||
       v_fundescr.size() == 0 ||
